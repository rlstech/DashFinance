...
FROM (
    -- VwDesembolsoAPagar com data correta:
    -- status=0 (Planejamento): DtPgto_des = data planejada (já é a prorrogação do planejamento)
    -- status=1 (Emitido): ISNULL(Prorroga, DataVencimento_Des) = vencimento prorrogado
    ...
    -- ContasPagas NOT in VwDesembolsoAPagar
...
for emp in sorted(data.keys()):
    emp_total = sum(float(r['Valor']) for obra in data[emp] for r in data[emp][obra])
    n_obras = len(data[emp])
    n_regs = sum(len(data[emp][o]) for o in data[emp])
    print(f'Empresa {emp}: {n_obras} obras, {n_regs} registros, R$ {emp_total:,.2f}')

grand = sum(float(r['Valor']) for emp in data for obra in data[emp] for r in data[emp][obra])
print(f'TOTAL GERAL: R$ {grand:,.2f} ({len(rows)} registros)')
